...
int MotionHandler(float* Mscan1[], float* Mscan2[], SysParams_Struct* SysParams,
	...
	*Pxx2[SysParams->SpectrogramFreqBins];
	...
	int SaveResults = 0;
	if (SaveResults == 1 && SysParams->FirstTimeMotion == 0) {
	...
	if (FirstGap == 1) {//this is the first interpolation between MotionStruct0 and  MotionStruct1
																				...
																				(SysParams->SpectrogramTimeBinsSingleMotion
																						...
																						+ PolynomCoeffs[2]
																										* (SysParams->SpectrogramTimeBinsSingleMotion + m
																												+ 1) + PolynomCoeffs[3];
			if(isHilbert==1){
				if (GapInterpolation[m] < SysParams->MinFreqPlusMinus)	//max(gap, minFreq);
					GapInterpolation[m] = SysParams->MinFreqPlusMinus;
			}
			else{//isHilbert==0
				if (GapInterpolation[m] < SysParams->MinFreqReal)	//max(gap, minFreq);
					GapInterpolation[m] = SysParams->MinFreqReal;//+0.01 to be sure that the event will not be cropped in the gap
		...
		for (m = 0; m < SysParams->GapLength; m++) {//implement polyval  i.e insert the missed timebins in the polynom.
			...
			if(isHilbert==1){
				...
				if (GapInterpolation[m] < SysParams->MinFreqReal)	//max(gap, minFreq);
					GapInterpolation[m] = SysParams->MinFreqReal;
...
int FeatureExtractionBasedCurves2(Motion_Struct2* MotionStruct,
	...
	FeatureSet->Plus->Edge2_50Precent_Fmax = SysParams->MinFreqPlusMinus;//=2.5 Hz   #11;
	FeatureSet->Minus->Edge2_50Precent_Fmax = SysParams->MinFreqPlusMinus; // #12;
	FeatureSet->Plus->Edge2_50Precent_AvgTopFive = SysParams->MinFreqPlusMinus; // #13
	FeatureSet->Plus->Edge2_50Precent_PeakToAvg = 1; //#15
	FeatureSet->Plus->Edge2_maxPeakFreq_fromTimeBinWithMaxFreq =
			SysParams->MinFreqPlusMinus; //#17
	FeatureSet->Minus->Edge2_maxPeakFreq_fromTimeBinWithMaxFreq =
			SysParams->MinFreqPlusMinus; //#18
	...
	FeatureSet->Plus->Max_Black_Curve = SysParams->MinFreqPlusMinus; //#37
	FeatureSet->Minus->Max_Black_Curve = SysParams->MinFreqPlusMinus; //#38
...
int Feature42_2(Motion_Struct2* MotionStruct, AllFeatures_Struct* FeatureSet) {
	...
	my_plan = fftwf_plan_dft_r2c_1d(FFTLength, fftin_local_ptr_complex, out,
			flags);
...
int Feature42(Motion_Struct2* MotionStruct, AllFeatures_Struct* FeatureSet,SysParams_Struct* SysParams) {
	...
	if (MotionStruct->EventPlusPassEvent == 1) {
		...
		if (sum_nonzero_preFFT_FiftyPrecent_Filtered != 0) {
			...
			ne10_fft_r2c_1d_float32_neon(FFTResult,(ne10_float32_t*)preFFTFiftyPrecentFiltered,SysParams->FFT_Feature42);
...
int ExtractFeatures2(Motion_Struct2* MotionStruct,
	...
	for (i = MotionStruct->EventStruct->chosenStart;
		...
		if (Type == 1 || Type == 2) {			//All or only Plus
			...
			if (MotionStruct->Edge2_Plus->FiftyPrecent_Filtered[i]
																> SysParams->MinFreqPlusMinus) {//calculate mean for fifty_precent_filtered without Fmin bins
	...
	if (Type == 1 || Type == 2) {			//All or only Plus
		...
		if (num_of_Edge2_noDC_Favg_Plus == 0)//equivalent to if isempty(Edge2_noDC) in matlab
			num_of_Edge2_noDC_Favg_Plus = SysParams->MinFreqPlusMinus;
...
int CurveLength(Motion_Struct2 *MotionStruct, SysParams_Struct *SysParams) {
	...
	for (i = 0; i < SysParams->SpectrogramTimeBins; i++) {
		...
		if (MotionStruct->Edge2->Peak_Filtered[i] > SysParams->MinFreqReal) { //put 1's in all the bins that pass the minValue otherwise stay 0
...
int HilbertSpectrogram2(float* Pxx2_Hilbert[], float* Pxx2_dB[], float *T2_dB,
	...
	SignalForFFT = (fftwf_complex*) fftwf_malloc(
			sizeof(fftwf_complex) * SysParams->SpectrogramFreqBinsHilbert);	//pay attention
	memset(SignalForFFT, 0,
			sizeof(fftwf_complex) * SysParams->SpectrogramFreqBinsHilbert);	//was in original
	...
	p = fftwf_plan_dft_1d(SysParams->SpectrogramFreqBinsHilbert, SignalForFFT,
...
int HilbertSpectrogram4(float* Pxx2_Hilbert[], float* Pxx2[], float* Pxx2_dB[],
	...
	double VecForMeanPlus[4],VecForMeanMinus[4],VecForMeanReal[4],MeanNoisePlus,MeanNoiseMinus,MeanNoiseRealPerTimeBin[SysParams->SpectrogramTimeBins];
	...
	int noiseThresh = 5;
	float MeanFactor= 1.0/ (SysParams->RangeWide);

	_Complex float *MscanIQ[SysParams->Nscans];

	float SpectrogramPerTimeBin[SysParams->SpectrogramFreqBinsHilbert];

	Pxx2_Plus_Struct Pxx2_Plus;
	Pxx2_Minus_Struct Pxx2_Minus;

	gsl_complex FFTResultForAbs;
	memset(SysParams->SignalForFFT, 0,
			sizeof(fftwf_complex) * SysParams->SpectrogramFreqBinsHilbert);	//was in original
	...
	for (i = 0; i < SysParams->SpectrogramFreqBinsHilbert / 2 + 1; i++) {//=101
		...
		for(int m=0;m < SysParams->SpectrogramTimeBins; m++ )
			Pxx2_Plus.Linear[i][m]=0;
	}

	for (i = 0; i < SysParams->SpectrogramFreqBinsHilbert / 2;i++){
		Pxx2_Minus.Linear[i] = (float *) malloc(
				SysParams->SpectrogramTimeBins * sizeof(float));
		Pxx2_Minus.dB[i] = (float *) malloc(
				SysParams->SpectrogramTimeBins * sizeof(float));
	...
	for (j = p1; j <= p2; j++) {	//relevant bins [p1,p2]
		...
		for (i = 0; i < SysParams->SpectrogramTimeBins; i++) {//take the next 30 points of slow for the FFT
			...
			for (m = 0; m < SysParams->winLength; m++) {//windowing with hamming
				...
				SysParams->SignalForFFT[m] = SysParams->Hamming[m]
																* MscanIQ[m + i * (SysParams->TimeShift)][j];
			}

			fftwf_execute(SysParams->FFT_HilbertSpectrogram);	//short time fft on 30 time bins

			//
			for (k = 1; k < SysParams->SpectrogramFreqBinsHilbert / 2 + 1; k++){//fill the Plus part of the spectrogram
				FFTResultForAbs.dat[0]=SysParams->FFTResult[k];//real part
				FFTResultForAbs.dat[1]=cimag(SysParams->FFTResult[k]);
			...
			FFTResultForAbs.dat[0]=SysParams->FFTResult[0];
			FFTResultForAbs.dat[1]=cimag(SysParams->FFTResult[0]);
			SpectrogramPerTimeBin[0]=gsl_complex_abs2(FFTResultForAbs);
			Pxx2_Plus.Linear[0][i] += (SpectrogramPerTimeBin[0]);//* MeanFactor;

			for(k = SysParams->SpectrogramFreqBinsHilbert / 2+1; k < SysParams->SpectrogramFreqBinsHilbert; k++) {//fill the Minus part of the spectrogram

				//				SpectrogramPerTimeBin[k] = powf(cabsf(FFTResult[k]), 2);
				FFTResultForAbs.dat[0]=SysParams->FFTResult[k];
				FFTResultForAbs.dat[1]=cimag(SysParams->FFTResult[k]);
				SpectrogramPerTimeBin[k]=gsl_complex_abs2(FFTResultForAbs);
				...
				Pxx2[SysParams->SpectrogramFreqBinsHilbert - k][i] +=
			...
			//
			//			for (k = 1 ;k <= SysParams->SpectrogramFreqBinsHilbert / 2;k++) {	//sum for the real spectrogram
			//
			//				Pxx2[k - 1][i] +=  (SpectrogramPerTimeBin[k]);
			//			}
...
//	printf (" %f sec\n", f1 );
	...
	for (i = 0; i < SysParams->SpectrogramTimeBins; i++) {
		...
		for (k = 0; k < SysParams->SpectrogramFreqBinsHilbert / 2 ; k++) {
	...
	//			+ (float) (tpStop.tv_usec) / 1000000)
	//																					- ((float) (tpStart.tv_usec) / 1000000);
	*T2_dB = 10 * log10(gsl_stats_mean(MeanNoiseRealPerTimeBin,1,SysParams->SpectrogramTimeBins))+ noiseThresh;
...
int HilbertSpectrogram44(float* Pxx2_Hilbert[], float* Pxx2[], float* Pxx2_dB[],
	...
	int k;
	int p2 = p1 + SysParams->RangeWide - 1;
	int LastBinsForNoisePlus=SysParams->SpectrogramFreqBinsHilbert / 2 + 1 - SysParams->noiseFreqBins;
	int LastBinsForNoiseMinus=SysParams->SpectrogramFreqBinsHilbert / 2 - SysParams->noiseFreqBins;
	fftwf_complex *SignalForFFT;
	...
	my_plan_fft = fftwf_plan_dft_1d(SysParams->SpectrogramFreqBinsHilbert,
	...
	for (j = p1; j <= p2; j++) {	//relevant bins [p1,p2]
		...
		for (i = 0; i < SysParams->SpectrogramTimeBins; i++) {//take the next 30 points of slow for the FFT
			...
			fftwf_complex test=FFTResult[0];;
			//
			for (k = 0; k < SysParams->SpectrogramFreqBinsHilbert / 2 + 1; k++){//fill the Plus part of the spectrogram
			...
			for(k = SysParams->SpectrogramFreqBinsHilbert / 2+1; k < SysParams->SpectrogramFreqBinsHilbert; k++) {//fill the Minus part of the spectrogram
				...
				Pxx2[SysParams->SpectrogramFreqBinsHilbert - k][i] +=
						(SpectrogramPerTimeBin[k]);
				//										/ (SysParams->RangeWide));//Calculate the average for the final REAL spectrogram
			}
	...
	for (i = 0; i < SysParams->SpectrogramTimeBins; i++) {
		...
		//
		//				for(k=LastBinsForNoisePlus;k< SysParams->SpectrogramFreqBinsHilbert / 2 + 1;k++){
		//					SumForMeanPlus += Pxx2_Plus.Linear[k][i];
		//				}
		//				for(k=LastBinsForNoiseMinus;k< SysParams->SpectrogramFreqBinsHilbert / 2 ;k++){
		//					SumForMeanMinus += Pxx2_Minus.Linear[k][i];
		//					SumForMeanRealPerTimeBin += Pxx2[k][i];
		//
		//				}
...
int Hilbertorg(float* Mscan[], _Complex float* MscanIQ[],
	...
	my_plan_fft = fftwf_plan_dft_r2c_1d(SysParams->Nbins,
			fftin_local_ptr_complex, MscanFFT, flags);

	my_plan_ifft = fftwf_plan_dft_1d(SysParams->Nbins, MscanFFT, MscanIFFT,
			FFTW_BACKWARD, FFTW_MEASURE);	//MAYBE ESTIMATEEEEE!!!!!
	for (k = 1; k < SysParams->Nbins ; k++)
		MscanFFT[k]=0;
...
int Hilbert(float* Mscan[], _Complex float* MscanIQ[],
	...
	for (k = 1; k < SysParams->Nbins ; k++)
		SysParams->MscanFFT[k]=0;
	for (i = 0; i < SysParams->Nscans; i++) {//implement FFT, erase the negative frequencies and implement IFFT.

		memcpy(SignalForFFT, Mscan[i], SysParams->Nbins * sizeof(Mscan[0][0]));	//prepare the signal for FFT

		fftwf_execute_dft_r2c(SysParams->FFT_Hilbert, SignalForFFT, SysParams->MscanFFT);	//FFT

		for (k = 1; k < SysParams->Nbins / 2; k++) {
			SysParams->MscanFFT[k] = 2 * SysParams->MscanFFT[k]; //in range K=[1,Nscans/2] as the algorithm says and normalize by N
		}

		//		for (k = 0; k < SysParams->Nbins ; k++) {
		//			printf("%d %f+%f\n",k,creal(SysParams->MscanFFT[k]),cimag(SysParams->MscanFFT[k]));
		//
		//		}

		fftwf_execute(SysParams->IFFT_Hilbert); //IFFT

		for (n = 0; n < SysParams->Nbins; n++) {
			MscanIQ[i][n] = conj(SysParams->MscanIFFT[n]) / (SysParams->Nbins); //use memcpy instead
		}
...
int MedianFilter(Edge2_Struct *Edge2, SysParams_Struct* SysParams,
	...
	gsl_movstat_workspace * w = gsl_movstat_alloc2(SysParams->MedianValue / 2,
			SysParams->MedianValue / 2 - 1);
...
int MedianFilterFor50Precent(Edge2_Struct *Edge2, Motion_Struct2 *MotionStruct,
...
int AbsOfFFTorigianl(float* Mscan[], float* Mscan_abs_FFT[],
	...
	memset(fftin_local_ptr_complex, 0, sizeof(fftin_local_ptr_complex));
	my_plan = fftwf_plan_dft_r2c_1d(SysParams->Nscans, fftin_local_ptr_complex,
			MscanFFT, flags);
...
int AbsOfFFT(float* Mscan[], float* Mscan_abs_FFT[],
	...
	for (j = 0; j < SysParams->Nbins; j++) {
		for (n = 0; n < SysParams->Nscans; n++) {
			SysParams->SignalForFFTABS[n] = Mscan[n][j];
		}
		fftwf_execute(SysParams->FFT_ABS);
		for (k = 0; k < SysParams->DFTLengthForPSD; k++) {//Length/2 because only one side is wanted

			FFTResultForAbs.dat[0]=SysParams->FFTResultABS[k];
			FFTResultForAbs.dat[1]=cimag(SysParams->FFTResultABS[k]);
			Mscan_abs_FFT[k][j]=gsl_complex_abs2(FFTResultForAbs);
		}
...
int MatchedFilter(float* Mscan[], SysParams_Struct* SysParams) {
	...
	ne10_float32_t fir_state_neon[FilterLength + SysParams->Nbins-1];
...
int GET_ROI(float *Mscan_abs_FFT[], SysParams_Struct* SysParams, int *p1) {
	...
	float Rangepower[SysParams->Nbins];
	float MaxValue;
	int LegnthOfSignal=SysParams->Nbins-SysParams->RangeWide+1;
	int i, j;


	ne10_fir_instance_f32_t SN; // An FIR "instance structure"
	int FilterLength = SysParams->RangeWide;
	ne10_float32_t fir_coeffs[SysParams->RangeWide];

	ne10_float32_t fir_state_neon[SysParams->RangeWide + SysParams->Nbins-1];
	ne10_float32_t FilteredSignal[SysParams->Nbins];

	for(i=0;i<SysParams->RangeWide;i++){
		fir_coeffs[i]=1;
	}
	...
	//implement with FIR filter a moving sum over RangeWIde and choose the maximum
	...
	for (i = 0; i < LegnthOfSignal; i++) {
		if (FilteredSignal[i+SysParams->RangeWide-1] > MaxValue) {
			MaxValue = FilteredSignal[i+SysParams->RangeWide-1];
			*p1 = i;
		}
	}
...
int Spectrogram2(float* Pxx2[], float* Pxx2_dB[], float *T2_dB, float* Mscan[],
	...
	my_plan = fftwf_plan_dft_r2c_1d(SysParams->DFTLengthForSpectrogram,
...
int AvgFilter(Edge2_Struct *Edge2, SysParams_Struct* SysParams, int IsHilbert) {
	...
	ne10_float32_t SignalForAvg[SignalLength];
	ne10_float32_t FilteredSignal[SignalLength];
	ne10_float32_t coeffs[SysParams->AvgValue];
	...
	if (IsHilbert == 1) { //in the ectrogram we contacte the last avgvalues of the previous motionstruct to the current
		...
		for (i = 0; i <= SignalLength; i++) { //contacte the end of the previous curve
			if (i < SysParams->AvgValue - 1)
				...
				SignalForAvg[i] = Edge2->FiftyPrecent[i - SysParams->AvgValue
													  + 1];
		}

		ne10_fir_init_float(&SN, SysParams->AvgValue, coeffs, fir_state_neon,
				SignalLength);

		ne10_fir_float_neon(&SN, SignalForAvg, FilteredSignal, SignalLength);
		memcpy(Edge2->FiftyPrecent_Filtered,
				FilteredSignal + (SysParams->AvgValue - 1),
				(SysParams->SpectrogramTimeBins) * sizeof(ne10_float32_t));
		//		for(i=0;i<SysParams->SpectrogramTimeBins;i++)
		//			printf("hilbert %d %f\n",i,Edge2->FiftyPrecent_Filtered[i]);
	}
		...
		ne10_fir_float_neon(&SN, Edge2->FiftyPrecent,
				Edge2->FiftyPrecent_Filtered, SysParams->SpectrogramTimeBins);
		//		for(i=0;i<SysParams->SpectrogramTimeBins;i++)
		//			printf("%d %f\n",i,Edge2->FiftyPrecent_Filtered[i]);
	}
